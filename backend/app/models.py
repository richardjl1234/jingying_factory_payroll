...
class Quota(Base):
    ...
    model_code = Column(String(10), ForeignKey("motor_models.model_code", ondelete="CASCADE"), nullable=False, index=True)
    ...
    __table_args__ = (
        UniqueConstraint('process_code', 'cat1_code', 'cat2_code', 'model_code', 'effective_date', name='_process_effective_date_uc'),
    )
...
class VSalaryRecord(Base):
    ...
    model_display = Column(String(150), nullable=True, comment="电机型号显示: 编码 (名称)")
