...
=== Testing navigation to Process Category 2 page ===")
        ...
        await logged_in_page.screenshot({'path': str(screenshots_dir / 'python_cat2_page.png')})
        
        # Check for Process Category 2 page elements
        page_content = await logged_in_page.content()
        
        # Check for Process Category 2 elements
        assert '分类2' in page_content or 'Process Category 2' in page_content
...
=== Testing Process Category 2 list display ===")
        ...
        if not found_table:
            print(f"No cat2 table found, but test continues")
