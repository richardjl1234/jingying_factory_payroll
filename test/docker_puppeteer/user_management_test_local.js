...
async function testUserManagement() {
  ...
  try {
    ...
    // 截图保存首页
    await page.screenshot({ path: `local_home_page.png` });
    console.log('保存首页截图');
    
    // 再次截图，确保页面稳定
    await waitForTimeout(1000);
    await page.screenshot({ path: `local_home_page_2.png` });
    ...
    // 截图保存用户管理页面
    await page.screenshot({ path: `local_user_management_page.png` });
    ...
    // 保存控制台日志到文件
    const fs = require('fs');
    fs.writeFileSync(`local_console_logs.txt`, consoleLogs.join('\n'));
